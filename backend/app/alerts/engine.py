...
def _build_liquidity(agent_name: str, forecast: ForecastResult, provider_names: dict[str, str]):
    ...
    return title, message_en, message_bn, evidence, forecast.confidence, forecast.confidence_note, forecast.data_quality


def _build_anomaly(agent_name: str, provider: Provider, anomaly):
    title, message_en, message_bn = anomaly_messages(agent_name, provider.id, provider.name, anomaly)
    evidence = {
    ...
    return title, message_en, message_bn, evidence, anomaly.confidence, anomaly.confidence_note, DataQuality.OK


def _build_data_quality(agent_name: str, provider: Provider, feed: DataFeedStatus):
    title, message_en, message_bn = data_quality_messages(agent_name, provider.id, provider.name)
    evidence = {
        "last_update_at": feed.last_update_at.isoformat(),
        "seconds_since_update": (datetime.utcnow() - feed.last_update_at).total_seconds(),
        "note": feed.note,
    }
    return title, message_en, message_bn, evidence, ConfidenceLevel.LOW, "Feed has not reported recently.", DataQuality.DEGRADED
...
def _maybe_create_alert(
    ...
    *,
    ...
    title, message_en, message_bn, evidence, confidence, confidence_note, data_quality = build()
