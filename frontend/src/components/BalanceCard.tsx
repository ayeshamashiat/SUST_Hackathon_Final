...
function statusStripClass(forecast: ForecastOut | undefined) {
  if (!forecast || forecast.status === "INSUFFICIENT_DATA") return "bg-slate-700";
  ...
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-900 overflow-hidden">
      ...
      <div className="p-4 space-y-3">
        ...
        {feedUpdatedAt && (
          <div className="text-xs text-slate-500">feed updated {formatTime(feedUpdatedAt)}</div>
        )}
        ...
        {forecast && (
          <div className="pt-2 border-t border-slate-800 space-y-1.5">
            ...
            {forecast.status === "AT_RISK" && forecast.projected_shortage_at ? (
              ...
              <p className="text-sm text-slate-400">Stable based on recent activity.</p>
            ) : (
